  ...
  describe('Check apex trigger creation', () => {
    test
      ...
      .it('should create foo trigger using ApexTrigger template and default output directory', (ctx) => {
        ...
        assert.fileContent(path.join(process.cwd(), 'foo.trigger'), 'trigger foo on SOBJECT (before insert)');
      });
    test
      .withOrg()
      .withProject()
      ...
      .it('should create foo trigger with a targetpath and sobject set', (ctx) => {
        assert.file([
    ...
    test
      ...
      .it('should override foo trigger with a different sobject and triggerevent', (ctx) => {
        assert.file(['foo.trigger', 'foo.trigger-meta.xml']);
        assert.fileContent('foo.trigger', 'trigger foo on override (after insert)');
      });
    ...
    test
      ...
      .it('should create foo trigger in custom folder name that has a space in it', (ctx) => {
        assert.file([path.join('classes create', 'foo.trigger'), path.join('classes create', 'foo.trigger-meta.xml')]);
  ...
  describe('Check that all invalid name errors are thrown', () => {
    test
      ...
      .it('should throw a missing trigger name error', (ctx) => {
        expect(ctx.stderr).to.contain(messages.getMessage('MissingTriggernameFlag'));
      });
