  ...
  describe('Check project creation', () => {
    test
      ...
      .it('should create project with default values and foo name', (ctx) => {
        ...
        assert.fileContent(path.join('foo', 'README.md'), messages.getMessage('StandardReadMe'));
    ...
    test
      ...
      .it(
        ...
        (ctx) => {
          assert.file([path.join('test outputdir', 'foo', 'config', 'project-scratch-def.json')]);
          assert.file([path.join('test outputdir', 'foo', 'README.md')]);
          assert.file([path.join('test outputdir', 'foo', 'sfdx-project.json')]);
          for (const file of vscodearray) {
            assert.file([path.join('test outputdir', 'foo', '.vscode', `${file}.json`)]);
          }
          assert.file([path.join('test outputdir', 'foo', 'force-app', 'main', 'default', 'lwc', '.eslintrc.json')]);
          assert.file([path.join('test outputdir', 'foo', 'force-app', 'main', 'default', 'aura', '.eslintrc.json')]);
          for (const file of filestocopy) {
            assert.file([path.join('test outputdir', 'foo', file)]);
          }
          for (const folder of standardfolderarray) {
            assert(fs.existsSync(path.join('test outputdir', 'foo', 'force-app', 'main', 'default', folder)));
          }
        }
      );
    ...
    test
      ...
      .it('should not create duplicate project in the directory where command is executed', (ctx) => {
        assert.file(path.join('test outputdir', 'duplicate-project-test', 'force-app'));
    ...
    test
      ...
      .it(
        ...
        (ctx) => {
          assert.file([path.join('foo-project', 'config', 'project-scratch-def.json')]);
          ...
          assert.file([path.join('foo-project', 'force-app', 'main', 'default', 'lwc', '.eslintrc.json')]);
          for (const file of filestocopy) {
            assert.file([path.join('foo-project', file)]);
          }
          for (const folder of standardfolderarray) {
            assert(fs.existsSync(path.join('foo-project', 'force-app', 'main', 'default', folder)));
          }
        }
      );
    ...
    test
      ...
      .it(
        ...
        (ctx) => {
          ...
          assert.fileContent(path.join('fooempty', 'sfdx-project.json'), '"namespace": "testnamespace",');
          assert.fileContent(path.join('fooempty', 'sfdx-project.json'), '"path": "empty",');
          assert.fileContent(path.join('fooempty', 'sfdx-project.json'), 'sourceApiVersion');
          for (const folder of emptyfolderarray) {
            assert(fs.existsSync(path.join('fooempty', 'empty', 'main', 'default', folder)));
          }
          assert.fileContent(path.join('fooempty', 'README.md'), '# Salesforce DX Project: Next Steps');
        }
      );
    ...
    test
      ...
      .it(
        ...
        (ctx) => {
          assert.file(path.join('fooempty', '.forceignore'));
          assert.fileContent(path.join('fooempty', 'sfdx-project.json'), '"namespace": "",');
          assert.fileContent(path.join('fooempty', 'sfdx-project.json'), '"path": "empty",');
          assert.fileContent(path.join('fooempty', 'sfdx-project.json'), 'sourceApiVersion');
          assert.fileContent(
    ...
    test
      ...
      .it('should create project with analytics1 name using analytics template and a manifest', (ctx) => {
        ...
        assert.fileContent(path.join('analytics1', 'sfdx-project.json'), '"path": "force-app",');
        assert.fileContent(path.join('analytics1', 'sfdx-project.json'), 'sourceApiVersion');
        const srcDir = path.join('analytics1', 'force-app', 'main', 'default');
        ...
        assert.fileContent(path.join('analytics1', '.vscode', 'extensions.json'), '"salesforce.analyticsdx-vscode"');
        assert.fileContent(path.join('analytics1', '.vscode', 'extensions.json'), '"salesforce.salesforcedx-vscode"');
        assert.fileContent(path.join('analytics1', 'config', 'project-scratch-def.json'), '"DevelopmentWave"');
        assert.fileContent(path.join('analytics1', 'manifest', 'package.xml'), '<name>WaveTemplateBundle</name>');
        assert.fileContent(path.join('analytics1', 'README.md'), '# Salesforce DX Project: Next Steps');
    ...
    test
      ...
      .it('should rename gitignore to .gitignore in standard template', (ctx) => {
        const srcPath = path.join(path.dirname(require.resolve('@salesforce/templates')), 'templates/project');
    ...
    test
      .command(['force:project:create', '-n', 'GitIgnoreTest2', '-t', 'analytics'])
      .it('should rename gitignore to .gitignore in analytics template', (ctx) => {
        const srcPath = path.join(path.dirname(require.resolve('@salesforce/templates')), 'templates/project');
        assert.noFile(path.join(srcPath, '.gitignore'));
        assert.file(path.join(srcPath, 'gitignore'));
  ...
  describe('project creation failures', () => {
    test
      ...
      .it('should throw invalid template name error', (ctx) => {
        expect(ctx.stderr).to.contain(messages.getMessage('MissingProjectname'));
      });
    test
      .withOrg()
      .withProject()
